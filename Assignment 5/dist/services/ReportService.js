...
const generateReport = (res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const employees = yield EmpModel_1.default.findAll({
            attributes: ["id", "name", "assignedShiftHours"],
        });
        console.log("employess", employees);
        const timesheets = yield TimeSheetModel_1.default.findAll({
            attributes: ["employeeId", "fromDate", "duration"],
        });
        const workLog = {};
        timesheets.forEach((timesheet) => {
            const { employeeId, fromDate, toDate, duration } = timesheet;
            const employee = employees.find((emp) => String(emp.dataValues.id) === String(employeeId));
